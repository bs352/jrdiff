 ...
 * @author bo.shen
 */
public class RDiffDeltaGenerator {
    ...
    private RDiffDeltaBlock deltaBlockToWrite = null;

    public void generate(InputStream sigFile, InputStream newFile, OutputStream delta) throws IOException, NoSuchAlgorithmException, DigestException {
        ...
        out.writeInt(RDiff.DELTA_MAGIC);

        createDelta(newFile, out);
    }

    private void createDelta(InputStream stream, DataOutputStream out) throws IOException, DigestException {
        int len;
        ...
        if (nextBufIdx > 0) {
            ...
            if (block == null) {
                writeBlock(out, new RDiffDeltaBlock(Arrays.copyOfRange(dataBuffer, 0, nextBufIdx)));
            }
            else {
                writeBlock(out, new RDiffDeltaBlock(block.getOffset(), nextBufIdx));
            }
        }
    ...
    private void update(byte[] buf, int offset, int len, DataOutputStream out) throws DigestException, IOException {
        for (int i = offset; i < offset + len; i++) {
            ...
            if (hitBlock != null) {
                ...
                if (diffBytes > 0) {
                    writeBlock(out, new RDiffDeltaBlock(Arrays.copyOfRange(dataBuffer, 0, diffBytes)));
                }
                writeBlock(out, new RDiffDeltaBlock(hitBlock.getOffset(), blockLength));
                nextBufIdx = 0;
            }
            else {
                if (nextBufIdx == dataBuffer.length) {
                    ...
                    writeBlock(out, new RDiffDeltaBlock(Arrays.copyOf(dataBuffer, packSize)));
                    nextBufIdx -= packSize;
    ...
    private void writeBlock(DataOutputStream out, RDiffDeltaBlock deltaBlock) throws IOException {
        if (deltaBlockToWrite != null) {
    ...
    private void writeBlockNow(DataOutputStream out, RDiffDeltaBlock deltaBlock) throws IOException {
        if (deltaBlock.isData()) {
            int len = byteCount(deltaBlock.getBlockLength());
            out.writeByte(RDiff.FLAG_DATA[len]);
            ...
            int len = byteCount(deltaBlock.getBlockLength());
            if (count <= 4) {
                out.writeByte(RDiff.FLAG_REF_4[len]);
                out.writeInt((int) deltaBlock.getOldOffset());
                writeBlockLength(out, deltaBlock, len);
            }
            else {
                out.writeByte(RDiff.FLAG_REF_8[len]);
    ...
    private void writeBlockLength(DataOutputStream out, RDiffDeltaBlock deltaBlock, int len) throws IOException {
        switch (len) {
            case 1:
